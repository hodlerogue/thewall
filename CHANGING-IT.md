...
contribution rendered flat instead of a dim `guest:music$ say ` in front of a
bright sentence — the one piece of typography this product spends anything on.
Addresses were text rather than the buttons they are. Blank lines collapsed
instead of holding their height. Reported as "even something as simple as saying
something looks different here than the real site", which it was.

The cap and `append` live there too, because `hints off` is applied on the way
...
not about an illustration. Reported as "that image was generated by image
generation, it's not an accurate depiction of what a link to a room would look
like." The section now points an `img` straight at `/music/opengraph-image`, so
the picture is drawn by the thing being described and cannot go stale; the
poster moved to the end, where it asserts nothing and its alt text is forbidden
by test from calling itself a screenshot.
