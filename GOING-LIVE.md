...
twenty-one. What the rest add: the column-scoped grants that close two console
...
lobby that uses its index. **None of their features work until they are applied**, and none of them
...
**Worked when:** `db-check.sh` shows all twenty-one as `applied`, every room has
