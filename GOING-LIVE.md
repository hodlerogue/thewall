...
twenty-two. What the rest add: the column-scoped grants that close two console
...
lobby that uses its index, and replies you can answer. **None of their features work until they are applied**, and none of them
...
**Worked when:** `db-check.sh` shows all twenty-two as `applied`, every room has
